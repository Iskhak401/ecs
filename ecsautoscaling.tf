...
resource "aws_appautoscaling_target" "chat_server_scale_target" {
  service_namespace  = "ecs"
  resource_id        = "service/${aws_ecs_cluster.chat_server_ecs_cluster.name}/${aws_ecs_service.chat_server_service.name}"
  scalable_dimension = "ecs:service:DesiredCount"
  max_capacity       = var.ecs_autoscale_max_instances
  min_capacity       = var.ecs_autoscale_min_instances
}
...
resource "aws_cloudwatch_metric_alarm" "chat_server_cpu_utilization_high" {
  alarm_name          = "app-${local.name}-CPU-Utilization-High-${var.ecs_as_cpu_high_threshold_per}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "CPUUtilization"
  ...
  dimensions = {
    ClusterName = aws_ecs_cluster.chat_server_ecs_cluster.name
    ServiceName = aws_ecs_service.chat_server_service.name
  }
  alarm_actions = [aws_appautoscaling_policy.chat_up.arn]
}
...
resource "aws_cloudwatch_metric_alarm" "chat_server_cpu_utilization_low" {
  alarm_name          = "app-${local.name}-CPU-Utilization-Low-${var.ecs_as_cpu_low_threshold_per}"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = "1"
  metric_name         = "CPUUtilization"
...
resource "aws_appautoscaling_policy" "chat_server_up" {
  name               = "chat-server-app-scale-up"
  service_namespace  = aws_appautoscaling_target.chat_server_scale_target.service_namespace
  resource_id        = aws_appautoscaling_target.chat_server_scale_target.resource_id
  scalable_dimension = aws_appautoscaling_target.chat_server_scale_target.scalable_dimension
...
resource "aws_appautoscaling_policy" "chat_server_down" {
  name               = "chat-scale-down"
  service_namespace  = aws_appautoscaling_target.chat_server_scale_target.service_namespace
  resource_id        = aws_appautoscaling_target.chat_server_scale_target.resource_id
  scalable_dimension = aws_appautoscaling_target.chat_server_scale_target.scalable_dimension
