...
module "chat_server_alb" {
  source  = "terraform-aws-modules/alb/aws"
  version = "~> 6.0"

  name = "${local.name}-${local.chat_server_resource}-alb"

  load_balancer_type = "application"

  vpc_id             = module.vpc.vpc_id
  subnets            = module.vpc.public_subnets
  security_groups    = [aws_security_group.chat_server_alb_sg.id]
...
#   }
  ...
  target_groups = [
    {
      name= "${local.name}-${local.chat_server_resource}-tg"
      backend_protocol     = "HTTP"
      backend_port         = 4000
      target_type          = "ip"
      deregistration_delay = 10
...
resource "aws_security_group" "chat_server_alb_sg" {
  name        = "${local.name}-${local.chat_server_resource}-alb-sg"
  description = "Allow HTTP to Load Balancer"
  vpc_id      = module.vpc.vpc_id
...
resource "aws_security_group" "chat_server_service_sg" {
  name        = "${local.name}-${local.chat_server_resource}-service-sg"
  description = "Allow access to ECS instance from Load Balancer"
  vpc_id      = module.vpc.vpc_id
