...
[
    {
      ...
      "secrets": [
        ...
        {
          "valueFrom": "${aws_secretsmanager_secret.peer_secret.id}:APPCONFIG__EnableRandomRecommendedFriends::",
...
#chat-server
resource "aws_ecs_cluster" "chat_server_ecs_cluster" {
  name = "${local.name}-${local.chat_server_resource}-ecs-cluster"
}

resource "aws_ecs_task_definition" "chat_server_task_definition" {
  family                   = "${local.name}-${local.chat_server_resource}-task-definition"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  cpu                      = var.task_cpu_unit
  memory                   = var.task_memory
...
[
    {
      ...
      "logConfiguration": {
        ...
        "secretOptions": null,
        "options": {
          "awslogs-group": "/ecs/${local.name}-${local.chat_server_resource}-task-definition",
          "awslogs-region": "${local.region}",
          "awslogs-stream-prefix": "ecs"
        }
      },
      ...
      "portMappings": [
        {
          "hostPort": ${var.chat_server_container_port},
          "protocol": "tcp",
          "containerPort": ${var.chat_server_container_port}
        }
      ],
      "command": null,
      "linuxParameters": null,
      ...
      "volumesFrom": [],
      "stopTimeout": null,
      "image": "${module.chat_server_ecr.repository_url}",
      "startTimeout": null,
      "firelensConfiguration": null,
      "dependsOn": null,
      "disableNetworking": null,
      ...
      "systemControls": null,
      "privileged": null,
      "name": "${local.name}-${local.chat_server_resource}-container"
    }
]
TASK_DEFINITION
...
data "aws_ecs_task_definition" "chat_server_task_definition" {
  task_definition = aws_ecs_task_definition.chat_server_task_definition.family
}
...
resource "aws_cloudwatch_log_group" "chat_server-logs" {
  name              = "/ecs/${local.name}-${local.chat_server_resource}-task-definition"
  retention_in_days = var.logs_retention_in_days
}